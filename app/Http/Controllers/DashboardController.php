    ...
    public function account_dashboard_index()
    {
        ...
        if (Auth::check()) {
            ...
            if (Auth::user()->type == 'super admin') {
                ...
                return $this->accountant_dashboard_index();
            } else {
                // Check if this is an accountant impersonating a company
                if (session('accountant_impersonating')) {
                    return $this->accountant_impersonation_dashboard();
                }
...
//                $user_deal   = $deals->pluck('deal_id')->toArray();
    ...
    public function accountant_dashboard_index()
    {
        if (Auth::check()) {
            if (Auth::user()->type == 'accountant') {
                $data = [];

                // Get connected companies (accepted invitations)
                $connectedCompanies = \App\Models\AccountantInvitation::where('accountant_id', Auth::id())
                    ->where('status', 'accepted')
                    ->with('company')
                    ->get();

                $data['total_companies'] = $connectedCompanies->count();
                $data['pending_invitations'] = \App\Models\AccountantInvitation::where('accountant_id', Auth::id())
                    ->where('status', 'pending')
                    ->count();

                // Get recent companies (last 5 accepted invitations)
                $data['recent_companies'] = $connectedCompanies->sortByDesc('accepted_at')->take(5);

                // For now, show placeholders for other data
                $data['active_projects'] = 0; // Would need to aggregate across all connected companies
                $data['monthly_revenue'] = 0; // Would need to aggregate across all connected companies

                // Placeholder data for upcoming tasks and activities
                $data['upcoming_tasks'] = collect([]); // Would be tasks from connected companies
                $data['recent_activities'] = collect([]); // Would be activities from connected companies

                return view('dashboard.accountant-dashboard', $data);
            } else {
    ...
    /**
     * Show restricted accountant dashboard when impersonating a company
     */
    public function accountant_impersonation_dashboard()
    {
        if (Auth::check() && session('accountant_impersonating')) {
            // Show only accounting-related data for impersonated accountant
            $data = [];

            // Financial data - same as regular accounting dashboard but restricted
            $data['latestIncome'] = Revenue::with(['customer'])->where('created_by', '=', \Auth::user()->creatorId())->orderBy('id', 'desc')->limit(5)->get();
            $data['latestExpense'] = Payment::with(['vender'])->where('created_by', '=', \Auth::user()->creatorId())->orderBy('id', 'desc')->limit(5)->get();
            ...
            // Remove non-accounting data: goals, customer/vendor counts, storage limit
            $data['goals'] = collect([]); // Empty collection for accountant view
            $data['storage_limit'] = 0; // Not relevant for accountants

            // Add accountant-specific indicator
            $data['is_accountant_view'] = true;
