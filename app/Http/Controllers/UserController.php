...
class UserController extends Controller
{
    ...
    if ($settings['storage_setting'] == 'local') {
        $full_image_path = public_path($image_path);
    } else {
        $full_image_path = storage_path($image_path);
    }
    ...
    public function LoginWithCompany(Request $request, User $user, $id)
    {
        ...
        if ($user && auth()->check()) {
            ...
            if (auth()->user()->type === 'accountant') {
                // Assign limited accountant role for company access
                $accountantRole = Role::where('name', 'accountant-limited')->first();

                if (!$accountantRole) {
                    // Create the role if it doesn't exist
                    $accountantRole = Role::create(['name' => 'accountant-limited']);
                    // Assign only accounting-related permissions
                    $permissions = [
                ...
                // Temporarily assign the limited role to the company user for this session
                // We'll handle this in middleware instead
                session(['accountant_impersonating' => true, 'original_user' => auth()->id()]);
            }
    ...
    public function ExitCompany(Request $request)
    {
        ...
        session()->forget('accountant_impersonating');
        return redirect('/account-dashboard');
    }
