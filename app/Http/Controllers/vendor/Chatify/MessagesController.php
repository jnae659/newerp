...
//                    $file->storeAs("/" . config('chatify.attachments.folder'), $attachment);
    ...
    public function getContacts(Request $request)
    {
        ...
        // Get connected accountants/companies for chat restrictions
        $objUser = Auth::user();
        $connectedUsers = collect();

        if($objUser->type == 'accountant')
        {
            // Accountants can only chat with companies they have accepted invitations with
            $acceptedInvitations = \App\Models\AccountantInvitation::where('accountant_id', $objUser->id)
                ->where('status', 'accepted')
                ->with('company')
                ->get();

            $connectedUsers = $acceptedInvitations->pluck('company');
        }
        elseif($objUser->type == 'company')
        {
            // Companies can only chat with accountants they have accepted invitations with
            $acceptedInvitations = \App\Models\AccountantInvitation::where('company_id', $objUser->id)
                ->where('status', 'accepted')
                ->with('accountant')
                ->get();

            $connectedUsers = $acceptedInvitations->pluck('accountant');
        }
        else
        {
            // For other user types (admin, employee, etc.), show company members as before
            if($objUser->type == 'company')
            {
                $connectedUsers = User::where('type', '!=', 'client')->where('created_by', '=', $objUser->creatorId())->get();
            }
            else
            {
                $connectedUsers = User::where('type', '!=', 'client')->where('created_by', '=', $objUser->creatorId())->where('id', '!=', $objUser->id)->orWhere('id', '=', $objUser->creatorId())->get();
            }
        }

        $getRecords = null;
        foreach($connectedUsers as $record)
        {
            $getRecords .= view(
        ...
        return Response::json(
            [
                ...
                'allUsers' => $connectedUsers->count() > 0 ? $getRecords : '<br><p class="message-hint"><span>' . __('Your contact list is empty') . '</span></p>',
            ], 200
        );
    }
