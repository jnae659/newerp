...
@section('content')
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('register.store', ['plan' => $plan]) }}" class='needs-validation' novalidate>
            ...
            @endif
            @csrf
            <input type="hidden" name="type" value="{{ request('type', 'company') }}">
            @php $userType = request('type', 'company'); @endphp
                ...
                <div class="d-flex gap-4">
                    <div class="form-check">
                        <input class="form-check-input @error('accountant_type') is-invalid @enderror"
                               ...
                               {{ old('accountant_type', 'individual') == 'individual' ? 'checked' : '' }}
                               required>
                    ...
                    <div class="form-check">
                        <input class="form-check-input @error('accountant_type') is-invalid @enderror"
                               ...
                               {{ old('accountant_type') == 'firm' ? 'checked' : '' }}
                               required>
            ...
            <div class="">
                ...
                <div class="form-group mb-3">
                    ...
                    <select id="country" class="form-control @error('country') is-invalid @enderror" name="country" required="required">
                        <option value="">{{ __('Select Country') }}</option>
                        <option value="Saudi Arabia" {{ old('country') == 'Saudi Arabia' ? 'selected' : '' }}>{{ __('Saudi Arabia') }}</option>
                        <option value="UAE" {{ old('country') == 'UAE' ? 'selected' : '' }}>{{ __('UAE') }}</option>
