...
@push('script-page')
    <script>
        ...
        @if(!isset($is_accountant_view) || !$is_accountant_view)
        (function () {
            var options = {
        ...
        <div class="col-sm-12">
            <div class="row">
                <div class="col-xxl-7">
                                ...
                                <div
                                    ...
                                    <div class="info-icon">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <path d="M22.4998 15.0005C22.4982 12.4139 22.4974 9.82724 22.4966 7.24061C23.3517 6.94616 23.8404 5.99713 23.8764 5.09351C23.9123 4.1899 23.5888 3.31518 23.2702 2.46884C22.9335 1.57463 22.5175 0.59307 21.6445 0.20466C21.1827 -0.000809863 20.6599 -0.00410235 20.1544 -0.00389985C14.6727 -0.00172486 9.19094 0.000457645 3.7092 0.00263264C3.18945 0.00283515 2.64627 0.00875264 2.18748 0.25302C1.53228 0.60186 1.20771 1.34415 1.92997 2.03252C0.530479 3.02268 0.117079 4.05641 0.114267 5.14069C0.111987 6.01971 0.706394 6.86912 1.50099 7.24142C1.50065 9.82947 1.5003 12.4175 1.49995 15.0055C0.457165 14.8907 -0.0437672 16.0718 -0.0393272 16.9251C-0.0357297 17.5196 0.152048 18.1662 0.649729 18.4911C1.03714 18.7439 1.52928 18.7522 1.9919 18.7521C8.72274 18.7512 15.4536 18.7502 22.1845 18.7493C22.5386 18.7492 22.908 18.7457 23.2234 18.5848C23.7411 18.3207 23.9762 17.7009 24.0255 17.1218C24.1025 16.2182 23.6305 14.9101 22.4998 15.0005ZM2.99996 7.47414C3.39873 7.41762 3.8189 7.25008 4.14693 7.01711C4.35164 6.87173 4.6479 6.87173 4.85445 7.01785C5.74727 7.6492 7.00117 7.64993 7.89693 7.01711C8.10164 6.87173 8.39827 6.87173 8.60445 7.01785C9.49727 7.6492 10.7512 7.64993 11.6469 7.01711C11.8513 6.87173 12.1479 6.87173 12.3544 7.01785C13.2465 7.6492 14.5019 7.64993 15.3969 7.01711C15.5998 6.87246 15.8972 6.87136 16.1044 7.01785C16.9965 7.6492 18.2519 7.64993 19.1469 7.01711C19.3498 6.87246 19.6472 6.87136 19.8544 7.01785C20.1877 7.25374 20.5974 7.412 21 7.47414V15.0005H16.243C15.7768 13.6844 14.7163 12.6721 13.4119 12.238C13.9189 11.8251 14.25 11.2039 14.25 10.5005C14.25 9.25979 13.2407 8.25051 12 8.25051C10.7592 8.25051 9.74995 9.25979 9.74995 10.5005C9.74995 11.2039 10.081 11.8251 10.588 12.238C9.28381 12.6721 8.2234 13.6844 7.75726 15.0005H2.99995C2.99995 15.0005 2.99995 7.47414 2.99996 7.47414Z" fill="white"/>
                                            </g>
                                            <defs>
                    ...
                    @if(!isset($is_accountant_view) || !$is_accountant_view)
                    <!-- Accountant Management Card -->
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h5 class="mt-1 mb-0">{{__('Accountant Connections')}}</h5>
                            <a href="{{ route('accountant-invitations.index') }}" class="btn btn-sm btn-primary">
                                <i class="ti ti-plus"></i> {{__('Manage')}}
                            </a>
                        </div>
                        <div class="card-body">
                            @php
                                $connectedAccountants = \App\Models\AccountantInvitation::where('company_id', \Auth::id())
                                    ->where('status', 'accepted')
                                    ->with('accountant')
                                    ->get();
                                $pendingInvitations = \App\Models\AccountantInvitation::where('company_id', \Auth::id())
                                    ->where('status', 'pending')
                                    ->count();
                            @endphp

                            @if($connectedAccountants->count() > 0 || $pendingInvitations > 0)
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <h6 class="mb-0">{{ $connectedAccountants->count() }}</h6>
                                                <small class="text-muted">{{__('Connected Accountants')}}</small>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <h6 class="mb-0">{{ $pendingInvitations }}</h6>
                                                <small class="text-muted">{{__('Pending Invitations')}}</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                @if($connectedAccountants->count() > 0)
                                    <div class="table-responsive">
                                        <table class="table table-sm">
                                            <thead>
                                                <tr>
                                                    <th>{{__('Accountant')}}</th>
                                                    <th>{{__('Connected Since')}}</th>
                                                    <th>{{__('Actions')}}</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($connectedAccountants->take(3) as $invitation)
                                                    <tr>
                                                        <td>
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <div>
                                                                    <span class="fw-bold">{{ $invitation->accountant->name }}</span>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td>{{ $invitation->accepted_at->format('M j, Y') }}</td>
                                                        <td>
                                                            <a href="{{ route('accountant-invitations.index') }}" class="btn btn-sm btn-outline-primary">
                                                                {{__('Manage')}}
                                                            </a>
                                                        </td>
                                                    </tr>
                                        ...
                                        @if($connectedAccountants->count() > 3)
                                            <div class="text-center mt-2">
                                                <a href="{{ route('accountant-invitations.index') }}" class="text-primary">
                                                    {{__('View all accountants')}}
                                                </a>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            @else
                                <div class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        <h6 class="mb-2">{{__('No Accountant Connections')}}</h6>
                                        <p class="mb-3">{{__('Connect with certified accountants to help manage your books')}}</p>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <a href="{{ route('accountant.marketplace') }}" class="btn btn-primary btn-sm">
                                                <i class="ti ti-search"></i> {{__('Browse Marketplace')}}
                                            </a>
                                            <a href="{{ route('accountant-invitations.create') }}" class="btn btn-outline-primary btn-sm">
