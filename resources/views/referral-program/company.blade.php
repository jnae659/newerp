        ...
        <div class="col-sm-12">
            <div class="row">
                <div class="col-xl-3">
                    <div class="card sticky-top" style="top:30px">
                        <div class="list-group list-group-flush" id="useradd-sidenav">
                            ...
                            <a href="#" class="list-group-item list-group-item-action border-0 tab-link"
                                data-tab="link-analytics">{{ __('Link Analytics') }}
                                <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                            </a>
                            <a href="#" class="list-group-item list-group-item-action border-0 tab-link"
                ...
                <div class="col-xl-9">
                            ...
                            <div class="row">
                                <div class="col-md-5 mb-3 mb-md-0">
                                    <div class = "border rounded p-3 mb-0">
                                        <h4>{{ __('Refer ') . $companyName .  __(' and earn up to ') . (isset($setting) ? $setting->level1_percentage : '') . __('% per paid signup!') }}</h4>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($referral->status == 'registered')
                                                        <span class="badge bg-warning">{{ __('Registered') }}</span>
                                                    @else
                                                        <span class="badge bg-success">{{ __('Paid') }}</span>
                                                    @endif
                                                </td>
                                                <td>{{ $currency . $referral->commission_amount }}</td>
                                                <td>{{ \App\Models\Utility::dateFormat($referral->created_at) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                    ...
                    <div id="link-analytics" class="card tab-content d-none">
                        <div class="card-header">
                            <h5>{{ __('Link Analytics') }}</h5>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="card text-center">
                                        <div class="card-body">
                                            <h3 class="text-primary">{{ $clickStats['total_clicks'] ?? 0 }}</h3>
                                            <p class="text-muted mb-0">{{ __('Total Clicks') }}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="card text-center">
                                        <div class="card-body">
                                            <h3 class="text-success">{{ $clickStats['unique_clicks'] ?? 0 }}</h3>
                                            <p class="text-muted mb-0">{{ __('Unique Clicks') }}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="card text-center">
                                        <div class="card-body">
                                            <h3 class="text-info">{{ $clickStats['clicks_today'] ?? 0 }}</h3>
                                            <p class="text-muted mb-0">{{ __('Clicks Today') }}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="card text-center">
                                        <div class="card-body">
                                            <h3 class="text-warning">{{ $clickStats['clicks_this_week'] ?? 0 }}</h3>
                                            <p class="text-muted mb-0">{{ __('Clicks This Week') }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row mt-4">
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            @if($trafficSources->isNotEmpty())
                                                ...
                                                @endforeach
                                            @else
                                                <p class="text-muted">{{ __('No traffic data available yet.') }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="card">
                                        <div class="card-header">
                                            <h6>{{ __('UTM Sources') }}</h6>
                                        </div>
                                        <div class="card-body">
                                            @if($topUTMSources->isNotEmpty())
                                                @foreach($topUTMSources as $utm => $count)
                                                    <div class="d-flex justify-content-between mb-2">
                                                        <span>{{ $utm }}</span>
                                                ...
                                                @endforeach
                                            @else
                                                <p class="text-muted">{{ __('No UTM data available yet.') }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                        ...
                                        <div class="card-body table-border-style">
                                            <div class="table-responsive">
                                                <table class="table">
                                                    ...
                                                    <tbody>
                                                        @forelse($clickStats['recent_clicks'] ?? [] as $click)
                                                            <tr>
                                                                <td>{{ $click->clicked_at->format('M d, Y H:i') }}</td>
                                                                <td>{{ $click->ip_address }}</td>
                                                                <td><span class="badge bg-secondary">{{ $click->traffic_source }}</span></td>
                                                                <td>{{ $click->utm_source ?: '-' }}</td>
                                                            </tr>
                                                        @empty
                                                            <tr>
                                                                <td colspan="4" class="text-center text-muted">{{ __('No clicks recorded yet.') }}</td>
                                                            </tr>
                                                        @endforelse
                                                    </tbody>
                                                </table>
